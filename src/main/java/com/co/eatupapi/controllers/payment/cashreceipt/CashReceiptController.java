...
public class CashReceiptController {
    ...
    @Operation(
            ...
            description = "Retorna los recibos de caja de un sitio con paginación. Si se especifica invoiceId, filtra por factura."
    )

    @ApiResponse(responseCode = "200", description = "Lista obtenida exitosamente")

    @GetMapping
    public ResponseEntity<?> getCashReceipts(
            @Parameter(description = "ID del sitio") @PathVariable UUID locationId,
            @Parameter(description = "Filtrar por ID de factura (opcional)") @RequestParam(required = false) UUID invoiceId,
        ...
        if (invoiceId != null) {
            return ResponseEntity.ok(cashReceiptService.getByInvoice(locationId, invoiceId));
        }
